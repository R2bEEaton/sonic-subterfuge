...
We're not done yet! Instead of attacking, let's try two things that might help the model be less susceptible to genetic noise attacks like the one above!

## Train with Noise

Noise is often an issue for classification models (in any space). The samples in the IRMAS dataset are quite nice, recorded in 16-bit sterio wav format, sampled at 44.1kHz. The examples generated by the genetic algorithm sound like audio recorded on a bad microphone, like those typically present in laptops even today. If a bad recording is enough to decrease classification confidence, then the model was just asking to be fooled.

Let's try improving it by generating some noisy audio and supplementing it into the training set. When generating features for the training set, 30% of the time I added a random amount of gaussian noise.
...
This produced a lowered accuracy of 53.8% on the training set. Trying the same as the above examples:
...
This seems to have produced the same or worse results. Training with noise made the model less certain about unperturbed classifications, and often even more foolable! Perhaps the genetic algorithm was able to easily lock onto some pattern of gaussian noise since it was included in some of the training data. We need a new approach.
...
On average, this performed better. The model remained relatively confident about unperturbed signals, and then did not budge much after the genetic algorithm. There were some outliers, but I believe this is due to the input audio files having multiple intstruments playing (see the voice example).

# Thanks

If you made it this far, thanks! And thanks to Professor Cartwright for his guidance through this project.
